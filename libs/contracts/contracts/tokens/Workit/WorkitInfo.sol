...
/**
 * @title WorkitInfo
 * @dev A library providing constants related to the Workit token, including its
 * decimal precision, maximum supply, and various fund allocations.
 */
library WorkitInfo {
	/// @dev The number of decimal places the Workit token uses (18 decimals).
	uint256 public constant DECIMALS = 18;

	/// @dev Represents 1 unit of Workit in its smallest denomination, taking into account the DECIMALS constant.
	uint256 public constant ONE = 10 ** DECIMALS;

	/// @dev The maximum supply of the Workit token, which is 21 million tokens.
	uint256 public constant MAX_SUPPLY = 21_000_000 * ONE;

	/**
	 * @dev The amount of Workit tokens allocated for ecosystem distribution.
	...
	/**
	 * @dev The amount of Workit tokens allocated for the Initial Coin Offering (ICO).
