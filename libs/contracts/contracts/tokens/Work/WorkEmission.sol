...
/// @notice Emitted when an unsupported entity key is provided.
error InvalidEntity(string entity);
...
/**
 * @title WorkEmission
 * @dev A library for calculating token emission over time based on an epoch model with a decay rate.
 * @dev Uses PRBMathSD59x18 library for fixed-point math operations.
 */
library WorkEmission {
	using PRBMathSD59x18 for int256;

	/// @dev The decay rate per epoch, represented with 18 decimals (0.9998).
	int256 internal constant DECAY_RATE = 9998e14;

	/// @dev Initial emission at epoch 0, scaled to WorkInfo.DECIMALS precision.
	int256 internal constant E0 = 272972703685;
	...
	function throughEpochRange(
		...
		require(
			epochEnd > epochStart,
			"WorkEmission: epochEnd must be greater than epochStart"
		);
	...
	function epochDecayFactor(uint256 epoch) private pure returns (int256) {
		return PRBMathSD59x18.pow(DECAY_RATE, toInt256(epoch * 1e18));
	}
}
...
library Entities {
	...
	function get(
		...
		bytes32 entityName = keccak256(abi.encodePacked(entity));

		if (entityName == keccak256(abi.encodePacked("team"))) {
			return self.team;
		}
		if (entityName == keccak256(abi.encodePacked("growth"))) {
			return self.growth;
		}
		if (entityName == keccak256(abi.encodePacked("staking"))) {
			return self.staking;
		}
		if (entityName == keccak256(abi.encodePacked("liqIncentive"))) {
			return self.liqIncentive;
		}

		revert InvalidEntity(entity);
	}

	function reset(Value storage self, string memory entity) internal {
		bytes32 entityName = keccak256(abi.encodePacked(entity));
