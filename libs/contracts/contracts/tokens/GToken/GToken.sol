...
/// @notice This contract handles the minting of governance tokens (GToken) used in the Workit platform.
...
contract GToken is IGToken, SFT {
	...
	// keccak256(abi.encode(uint256(keccak256("workit.GToken.storage")) - 1)) & ~bytes32(uint256(0xff));
	...
	function initialize(Epochs.Storage memory _epochs) public initializer {
		__SFT_init("Workit Governance Token", "GToken");
