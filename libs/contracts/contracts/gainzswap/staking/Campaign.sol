...
contract Campaign is ICampaign, OwnableUpgradeable, IERC1155Receiver {
	...
	/// @custom:storage-location erc7201:gainzswap.campaign.storage
	...
	bytes32 internal constant CAMPAIGN_STORAGE_LOCATION =
		keccak256("gainzswap.campaign.storage") & ~bytes32(uint256(0xff));
	...
	function contribute(
		...
		uint256 amount = dedu._wrapEDU(msg.value);
		_contribute(amount, to, $.launchpad);
	}
	...
	function contributeWEDU(
		...
		uint256 deduAmt = wedu._wrapWEDU(dedu, amount);
		_contribute(deduAmt, to, $.launchpad);
	}
