	...
	CampaignFactory
{
	...
	/// @custom:storage-location erc7201:gainzswap.launchpad.storage
	...
	bytes32 private constant CONTROLLER_STORAGE_SLOT =
		keccak256("gainzswap.launchpad.storage") & ~bytes32(uint256(0xff));
		...
		returns (IGToken.Balance[] memory balances, uint256[] memory values)
	{
		...
		for (uint256 i; i < nonces.length; ++i) {
			...
			// Ensure GToken is LP containing gainzToken
			if (!balance.attributes.hasToken(gainzToken))
				revert GTokenNotSecurityDeposit(nonces[i], gainzToken);
	...
	function createCampaign(
		...
		address gainz = IRewards(IStaking($.staking).rewards()).gainz();
		(, uint256[] memory values) = _validateSecurityPayment(
			securityNonces,
			IGToken($.gToken),
			gainz,
			IGainz(gainz).epochs().currentEpoch()
		);
