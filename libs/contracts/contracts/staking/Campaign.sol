...
contract Campaign is ICampaign, OwnableUpgradeable, IERC1155Receiver {
	...
	/// @custom:storage-location erc7201:workitswap.campaign.storage
	...
	bytes32 internal constant CAMPAIGN_STORAGE_LOCATION =
		keccak256("workitswap.campaign.storage") & ~bytes32(uint256(0xff));
	...
	{
		bytes32 slot = CAMPAIGN_STORAGE_LOCATION;
		assembly {
			$.slot := slot
		}
	}
	...
	function _checkDHBARFunding(
		address dhbar,
		address fundingToken
	) internal pure returns (address) {
		if (dhbar != fundingToken)
			revert InvalidFundingToken(fundingToken, dhbar);
		return dhbar;
	}
	...
	function contribute(
		...
		CampaignStorage storage $ = _campaignStorage();
		address dhbar = _checkDHBARFunding(
			ILaunchpad($.launchpad).dHBAR(),
			$.listing.fundingToken
		);

		uint256 amount = dhbar._wrapHBAR(msg.value);
		_contribute(amount, to, $.launchpad);
	}

	function contributeWHBAR(
		uint256 amount,
		address to
	) external notExpired inStatus(Status.Funding) {
		CampaignStorage storage $ = _campaignStorage();
		...
		address whbar = ILaunchpad($.launchpad).WHBAR();

		uint256 dhbarAmount = whbar._wrapWHBAR(dhbar, amount);
		_contribute(dhbarAmount, to, $.launchpad);
	}
	...
	// TODO remove LEGACY CAMPAIGNS when contract deployed on educhain; Codex, and othe AI tools ask for this and ensure this before implementing
	/*//////////////////////////////////////////////////////////////
	                      LEGACY CAMPAIGNS 
	//////////////////////////////////////////////////////////////*/

	function _ensureLegacy() private view returns (CampaignStorage storage $) {
		$ = _campaignStorage();
		if (block.timestamp <= $.listing.deadline)
			revert CampaignNotExpired($.listing.deadline, block.timestamp);
		if (
			msg.sender != $.launchpad &&
			msg.sender != Launchpad($.launchpad).campaignMigrator()
		)
			revert NotAllowed(
				msg.sender,
				$.launchpad,
				Launchpad($.launchpad).campaignMigrator()
			);
		uint256 securityCount = ILaunchpad($.launchpad)
			.getSecurityGTokens(address(this))
			.length;
		uint256 campaignTokens = campaignSupply();
		if (securityCount != 0 || campaignTokens != 0)
			revert CampaignFundsFound(securityCount, campaignTokens);
	}
	...
	function resolveCampaign(
		...
		address to
	) external inStatus(Status.Pending) returns (Status) {
		CampaignStorage storage $ = _ensureLegacy();

		if (finalStatus != Status.Failed && finalStatus != Status.Success)
			revert InvalidFinalStatus(finalStatus);
		if (to == address(0)) revert ZeroAddress(to);
		if (msg.sender != $.launchpad)
			revert UnauthorizedCaller(msg.sender, $.launchpad);

		if (pendingContribution == 0)
			revert PendingContributionZero(pendingContribution);
		ILaunchpad($.launchpad).mint(to, pendingContribution);
