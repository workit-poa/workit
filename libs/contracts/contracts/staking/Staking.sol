...
import {dHBARLib} from "../libraries/dHBARLib.sol";
...
contract Staking is IStaking, Initializable, OwnableUpgradeable {
	...
	using dHBARLib for address;

	/*//////////////////////////////////////////////////////////////
	                            STORAGE
	//////////////////////////////////////////////////////////////*/
	/// @custom:storage-location erc7201:workitswap.governance.staking.storage
	...
	bytes32 internal constant STAKING_STORAGE_LOCATION =
		keccak256("workitswap.governance.staking.storage") &
	...
	function initialize(
		...
		$.rewards = _rewards;
		$.workit = IRewards(_rewards).workit();
		$.gToken = IRewards(_rewards).gToken();
		$.epochs = IWorkit($.workit).epochs();
	}
	...
	function _stakeLiquidity(
		...
		uint256 amountToDHBAR = _selectInputAmountForDHBAR(
			liqInfo.token0,
			liqInfo.token1,
			amt0,
			amt1,
		...
		liqInfo.liqValue = _computeLiqValue(amountToDHBAR, pathToDHBAR);

		IWorkit($.workit).mintWorkit(); // Required to get updated IRewards.rewardPerShare value
	...
	function _stakeDHBARLiquidity(
		...
		address[] memory pathToDHBAR = new address[](1);
		pathToDHBAR[0] = dHBAR();
		...
		_stakeLiquidity(liqInfo, pathToDHBAR, to, epochsLocked);
	}
	...
	function stakeHBARLiquidity(
		...
		require(msg.value > 0, "Staking: zero HBAR");

		uint256 dHBARAmount = _wrapHBAR(msg.value);
		_pullToken(token, tokenAmount);

		_stakeDHBARLiquidity(
			token,
			dHBARAmount,
			tokenAmount,
			dHBARMin,
	...
	function stakeWHBARLiquidity(
		...
		uint256 dHBARAmount = _wrapWHBAR(whbarAmount);
		_pullToken(token, tokenAmount);
	...
	function _optimalSwapDHBAR(
		address pair,
		uint256 totalDHBAR,
		uint256 tokenMinOut
	)
		internal
		returns (address token, uint256 dHBARAmount, uint256 tokenAmount)
	{
		if (totalDHBAR == 0) revert ZeroAmount();

		// Perform optimal swap (dHBAR → token)
		(dHBARAmount, tokenAmount, token) = _optimalSwap(
			dHBAR(),
			totalDHBAR,
			tokenMinOut,
			pair
	...
	function stakeHBARLiquidityIn(
		...
		require(msg.value > 0, "Staking: zero HBAR");
		uint256 totalDHBAR = _wrapHBAR(msg.value);
		(
			address token,
			uint256 dHBARAmount,
			uint256 tokenAmount
		) = _optimalSwapDHBAR(pair, totalDHBAR, tokenMin);

		_stakeDHBARLiquidity(
			token,
			dHBARAmount,
			tokenAmount,
			dHBARMin, // DHBAR Min amount
	...
	function stakeWHBARLiquidityIn(
		...
		uint256 totalDHBAR = _wrapWHBAR(whbarAmount);
	...
	function _computeLiqValue(
		...
		StakingStorage storage $ = _stakingStorage();
		require(
			pathToDHBAR.length >= 1 &&
				pathToDHBAR[pathToDHBAR.length - 1] == dHBAR(),
			"Invalid Path to dhbar"
		);
		if (pathToDHBAR.length >= 2) {
			require(
				pathToDHBAR[0] != pathToDHBAR[pathToDHBAR.length - 1],
				"Invalid Path to dhbar"
			);
		}

		value = amountIn;
		for (uint256 i; i < pathToDHBAR.length - 1; ++i) {
			value = $.oracle.consult(pathToDHBAR[i], value, pathToDHBAR[i + 1]);
		}
		value *= 2;

		require(value > 0, "Invalid liquidity for DHBAR");
	}

	function _selectInputAmountForDHBAR(
		address tokenA,
		address tokenB,
		uint256 amountA,
		uint256 amountB,
		...
		require(pathToDHBAR.length >= 1, "InvalidPath");

		address input = pathToDHBAR[0];

		if (input == tokenA) return amountA;
		if (input == tokenB) return amountB;

		revert("InvalidDHBARPath");
	}
	...
	*/
	...
	function _wrapHBAR(uint256 amount) internal returns (uint256) {
		return dHBAR()._wrapHBAR(amount);
	}

	function _wrapWHBAR(uint256 amount) internal returns (uint256) {
		return WHBAR()._wrapWHBAR(dHBAR(), amount);
	}

	function dHBAR() public view returns (address) {
		return IUniswapV2Router(_stakingStorage().router).dHBAR();
	}

	function WHBAR() public view returns (address) {
		return IUniswapV2Router(_stakingStorage().router).WHBAR();
	}
