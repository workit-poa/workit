...
interface ICampaign {
	...
	error InvalidFundingToken(address provided, address dhbar);
	...
	/// @notice Native HBAR contribution (dHBAR campaigns only)
	function contribute(address to) external payable;

	/// @notice WHBAR contribution (dHBAR campaigns only)
	function contributeWHBAR(uint256 amount, address to) external;
	...
	/// @notice Launchpad-driven resolution (backward compatibility)
