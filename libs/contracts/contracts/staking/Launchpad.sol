	...
	CampaignFactory
{
	...
	uint256 public constant TARGET_GOAL_DHBAR_VALUE = 2 * SECURITY_GTOKEN_AMOUNT;
	uint256 public constant SECURITY_GTOKEN_MIN_EPOCHS_TO_EXPIRE = 90; // 90 epochs
	string private constant LAUNCHPAD_NAME = "Workit Launchpad";
	...
	/// @custom:storage-location erc7201:workitswap.launchpad.storage
	struct LaunchpadStorage {
		address factory;
		address gToken;
		address staking;
		mapping(address => address[]) fundingTokenToDHBAR;
	...
	bytes32 private constant CONTROLLER_STORAGE_SLOT =
		keccak256("workitswap.launchpad.storage") & ~bytes32(uint256(0xff));
	...
	function deployPair() external onlyCampaigns {
		...
		IStaking($.staking).stakeLiquidityIn(
			pair,
			liquidity,
			$.fundingTokenToDHBAR[listing.fundingToken],
			address(campaign),
			listing.lockEpochs
		);
	}
	...
	function _validateListing(
		...
		// --- Funding goal check in DHBAR ---
		address[] memory fundingPathToDHBAR = $.fundingTokenToDHBAR[
			listing.fundingToken
		];
		if (fundingPathToDHBAR.length == 0)
			revert NoFundingPathToDHBAR(listing.fundingToken);

		uint256 goalDHBARValue;
		if (fundingPathToDHBAR.length == 1) {
			goalDHBARValue = listing.goal;
		} else {
			...
			goalDHBARValue = amountsOut[amountsOut.length - 1];
		}
		if (goalDHBARValue < TARGET_GOAL_DHBAR_VALUE)
			revert InsufficientGoalDHBAR(goalDHBARValue, TARGET_GOAL_DHBAR_VALUE);
		...
		returns (IGToken.Balance[] memory balances, uint256[] memory values)
	{
		...
		for (uint256 i; i < nonces.length; ++i) {
			...
			// Ensure GToken is LP containing workitToken
			if (!balance.attributes.hasToken(workitToken))
				revert GTokenNotSecurityDeposit(nonces[i], workitToken);
	...
	function createCampaign(
		...
		address workit = IRewards(IStaking($.staking).rewards()).workit();
		(, uint256[] memory values) = _validateSecurityPayment(
			securityNonces,
			IGToken($.gToken),
			workit,
			IWorkit(workit).epochs().currentEpoch()
		);
	...
	) external onlyCampaignMigrator {
		LaunchpadStorage storage $ = _launchpadStorage();

		if (listing.fundingToken == address(0))
			revert ZeroFundingToken(listing.fundingToken);
		if (listing.campaignToken == address(0))
			revert ZeroCampaignToken(listing.campaignToken);
		address existingCampaign = campaignByTokens(
			listing.fundingToken,
			listing.campaignToken
		);
		if (existingCampaign != address(0))
			revert CampaignAlreadyExists(existingCampaign);

		(address campaign, ) = _createCampaign(creator, $.gToken, listing);
		ICampaign(campaign).resolveCampaign(
			status,
			pendingContribution,
			$.campaignMigrator
		);
	...
	function setTokenPathToDHBAR(address[] calldata path) external onlyOwner {
		LaunchpadStorage storage $ = _launchpadStorage();

		if (path.length == 0) revert EmptyPath();

		address dhbar = IStaking($.staking).dHBAR();

		// Case 1: already DHBAR, no swap needed
		if (path.length == 1) {
			if (path[0] != dhbar) revert SinglePathMustBeDHBAR(path[0], dhbar);
		} else {
			// Case 2: multi-hop path ending in DHBAR
			address outputToken = path[path.length - 1];
			if (outputToken != dhbar)
				revert InvalidOutputToken(outputToken, dhbar);
		...
		// Below is not required since we can update a token's pathToDHBAR
		// require(
		...
		$.fundingTokenToDHBAR[inputToken] = path;
	}
	...
	function removeAllowedFundingToken(address token) external onlyOwner {
		...
		// Delete swap path to DHBAR
		delete $.fundingTokenToDHBAR[token];
	}
	...
	function dHBAR() public view returns (address) {
		return IStaking(_launchpadStorage().staking).dHBAR();
	}

	function WHBAR() public view returns (address) {
		return IStaking(_launchpadStorage().staking).WHBAR();
	}
