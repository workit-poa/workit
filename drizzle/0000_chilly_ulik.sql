...
CREATE TABLE "users" (
	...
	"hedera_public_key_fingerprint" varchar(64),
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
ALTER TABLE "refresh_tokens" ADD CONSTRAINT "refresh_tokens_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "email_otp_challenges_email_idx" ON "email_otp_challenges" USING btree ("email");--> statement-breakpoint
...
CREATE UNIQUE INDEX "users_twitter_id_unique" ON "users" USING btree ("twitter_id");--> statement-breakpoint
